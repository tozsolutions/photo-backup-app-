            ...
            {session.isActive && (
              <div className="mt-3 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                <p className="text-sm text-blue-700">
                  📱 Cihazınızdan dosyalar yedekleniyor. Lütfen WiFi
                  bağlantınızı kesmeyin.
                </p>
              </div>
            )}
            ...
            {!session.isActive && session.progress === 100 && (
              <div className="mt-3 p-3 bg-green-50 border border-green-200 rounded-lg">
                <p className="text-sm text-green-700">
                  ✅ Yedekleme başarıyla tamamlandı! Dosyalarınız güvenle
                  kaydedildi.
                </p>
              </div>
            )}
              ...
              session.progress > 0 && (
                <div className="mt-3 p-3 bg-red-50 border border-red-200 rounded-lg">
                  <p className="text-sm text-red-700">
                    ❌ Yedekleme işlemi yarıda kesildi. WiFi bağlantınızı
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-center">
          ...
          <div>
            <p className="text-2xl font-bold text-gray-800">
              {sessions.reduce(
                (total, session) => total + session.totalFiles,
                0
              )}
            </p>
